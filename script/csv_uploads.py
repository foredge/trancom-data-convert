...
def csv_converter(data, exist_records):
    ...
    record = [
        ...
        '',  #
        '',  #
        # 2020/10/27 CSVにカラムが追加され、この箇所に変更はなかったはずだが、列番号([36]+[42]+[48]) を参照していて、値に「00:00」のフォーマットだったのでfloat変換でエラー出始めた。なぜ今までいけてたのか不明。
        overwork_time(float(data[34]) + float(data[40]) + float(data[46])),  # 残業(H) 以下のルールで入力する。([35]+[41]+[47])時間程度 ただし、「0時間程度」となる場合は以下を入力する。 原則ありません。
        '',  #
        data[18],  # 休日・休暇
